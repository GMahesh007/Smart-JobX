...
export default function Jobinput() {
  ...
  const handleNumQuestionsChange = (num) => {
    ...
    setQuestions((prevQuestions) => {
      const updatedQuestions = [...prevQuestions];
      if (number > updatedQuestions.length) {
        for (let i = updatedQuestions.length; i < number; i++) {
          updatedQuestions.push({ question: '', options: ['', '', '', ''], correctOption: '' });
        }
      } else if (number < updatedQuestions.length) {
        updatedQuestions.length = number;
      }
      return updatedQuestions;
    });
  };
    ...
    try {
      // Loop 10 times for 100 questions
        const response = await axios.post(
          "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyCd36O8Ysi7L1MBMbGkLN-4DLfD0NVHgds",
          {
            contents: [
              {
                parts: [
          ...
          <div className="space-y-2">
            ...
            <RadioGroup value={inputMethod} onValueChange={setInputMethod} className="flex space-x-4">
              ...
              <div>
                ...
                <Label htmlFor="gemini">Use AI</Label>
              </div>
            </RadioGroup>
          </div>
          ...
          {inputMethod === 'gemini' && (
            <>
              ...
              <Button onClick={generateQuestions} disabled={loading}>
                {loading ? "Generating Questions..." : `Generate 10 Questions on "${topic}" with AI`}
              </Button>

              {/* Render generated questions for review */}
              {questions.length > 0 && (
