...
export default function Exam() {
 ...
 useEffect(() => {
  ...
  const renderButton = () => {
    ...
    //   const mailtoLink = `mailto:${job.subRecruiterEmail}?subject=Smart JobX Referral Request for ${job.jobName}`;
    ...
    // 
    if (score >= job.referral) {
  const mailtoLink = `mailto:${job.subRecruiterEmail}?subject=Smart JobX Referral Request for ${job.jobName}&body=${encodeURIComponent(
    `Hi ${job.subRecruiterName || "Recruiter"},

I have completed the assessment for the position of "${job.jobName}" on Smart JobX and qualified for a referral.

✅ My score: ${score} / ${job.questions.length}

Kindly consider referring me for this opportunity.

Best regards,  
${SeekerInfo.name || "Your Name"}  
Smart JobX Candidate`
  )}`;
